...
public class ImageMainActivity extends ImagesBaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		ImagesBaseActivity.MAX_SEND = getIntent().getIntExtra("MAX_SEND",9);
	...
	public void loadForTask(int type){
		HashMap<String, ImageFolderEntity> mDataFolder = new HashMap<>();
		...
		if (!mDataFolder.isEmpty()) {
			List<ImageFolderEntity> mDataDDR = new ArrayList<>(mDataFolder.values());
